...
<section class="content-header">
  ...
  <p class="text-right"><a href="{{ route('member.teams.create') }}" class="btn btn-success">@lang('quickadmin.qa_add_new')</a></p>
</section>
<!-- Main content -->
  ...
  <div class="row">
   ...
   <div class="content-header-with-cell">
      <div class="custom-search-block">
       	 {!! Form::open(['method' => 'get','name'=>'team-form', 'class'=>'form-horizontal validation-form','data-parsley-validate','route' => ['member.teams.index']]) !!}
          <div class="input-group input-group-lg">
           {!! Form::text('search', old('search'), ['class' => 'search', 'class' => 'form-control', 'placeholder' => 'Search Team','previous'=>'previous']) !!}
        ...
        <div class="box-body">
          <table id="teamgrid" class="table table-bordered table-striped">
            ...
            <tr data-entry-id="{{ $data->team_id }}">
              ...
              <td field-key='last_name'>{{ $data->address_1 }} {{ $data->address_2 }}</td>
              <td field-key='title'>{{ $data->city->city or 'None'}}</td>
              <td field-key='title'>{{ $data->state->name or '' }}</td>
              <td field-key='title'>{{ $data->zip or '' }}</td>
              <td field-key='title'>{{ ucfirst($data->approval_status) }}</td>
              <td field-key='created_at'>{{ DateFacades::dateFormat($data->created_at,'formate-4') }}</td>
              <td>
               @can('member_tryouts')
               <a href="{{url('member/tryout/'.$data->team_id)}}" class="btn btn-xs btn-info">@lang('quickadmin.tryout.title')</a>
               @endcan
               <a href="{{ route('member.teams.edit',[$data->team_id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')
               </a>
               {!! Form::open(array(
                'style' => 'display: inline-block;',
                'method' => 'DELETE',
                'onsubmit' => "return confirm('".trans("quickadmin.qa_are_you_sure")."');",
                'route' => ['member.teams.destroy', $data->team_id])) !!}
