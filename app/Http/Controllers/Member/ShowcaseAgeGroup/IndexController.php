<?php
namespace App\Http\Controllers\Member\ShowcaseAgeGroup;
...
class IndexController extends Controller{
    ...
    public function index(Request $request)
	{
		...
		return view('member.showcase_age_groups.index',compact('ageGroups','paging'));
    }
	...
	public function save(Request $request)
	{
		...
		if(isset($result['id']))
		{
			...
			if($result['success']==false){
			    return view('member.showcase_age_groups.create',compact('statusList','ageGroup','module_id'))->withErrors($result['message']);
			}else{
			 	$request->session()->flash('success', $result['message']);
			 	return view('member.showcase_age_groups.create',compact('statusList','ageGroup','module_id'));
			}
		}else{
			if($result['success']==false){
			    return view('member.showcase_age_groups.create',compact('statusList','module_id'))->withErrors($result['message']);
			}else{
				$request->session()->flash('success', $result['message']);
				return view('member.showcase_age_groups.create',compact('statusList','module_id'));
			}
		}
	}
	...
	public function edit($id)
	{
       ...
       $statusList = $this->_ageGroupHelper->getStatusDropdown();
       $module_id = $this->_showcaseOrProspectHelper->getModuleId();
	   return view('member.showcase_age_groups.create',compact('ageGroup','statusList','module_id'));
    }
    ...
    public function destroy($id)
	{
		...
		if($isdelete){
			 return redirect()->route('member.showcase_age_groups.index')->with('success','Age Group Deleted Successfully.');
		}else{
			 return redirect()->route('member.showcase_age_groups.index')->with('error','Age Group Is Not Deleted Successfully.');
		}
    }
}
